...
exports.evaluate = function (opts, callback) {
    ...
    function evaluate(arr, cb, bTopLevel = undefined) {
        count++;
        if (count % 100 === 0) // avoid extra long call stacks to prevent Maximum call stack size exceeded
            return setImmediate(evaluate, arr, cb, undefined);
        if (fatal_error)
            return cb(false);
        ...
        switch (op) {
            ...
            case 'hypot':
                ...
                async.eachSeries(arr[1], function (param, cb2) {
                    ...
                    evaluate(Decimal[op].apply(Decimal, vals), cb, undefined);
                });
                break;
            ...
            function getDataFeed(params, cb) {
                ...
                if (params.min_mci) {
                    ...
                    if (!(/^\d+$/.test(min_mci as string) && ValidationUtils.isNonnegativeInteger(parseInt(min_mci as string))))
                        return cb("bad min_mci: "+min_mci);
                    min_mci = parseInt(min_mci as string);
                }
                var ifseveral = 'last';
                if (params.ifseveral){
                    ...
                    function(param_name, cb2){
                        evaluate(params[param_name].value, function(res){
                            ...
                            if (!isValidValue(res) || typeof res === 'boolean' && param_name !== 'ifnone')
                                return setFatalError('bad value in data feed: '+res, cb2, undefined);
                            if (Decimal.isDecimal(res))
                                res = toDoubleRange(res);
                    ...
                    function(param_name, cb2){
                        evaluate(params[param_name].value, function(res){
                            ...
                            if (!isValidValue(res) || typeof res === 'boolean')
                                return setFatalError('bad in-df param', cb2, undefined);
                            if (Decimal.isDecimal(res))
                                res = toDoubleRange(res);
                            evaluated_params[param_name] = {
                                operator: params[param_name].operator,
                    ...
                    function(param_name, cb2){
                        evaluate(params[param_name].value, function(res){
                            ...
                            if (res instanceof wrappedObject)
                                res = true;
                            if (!isValidValue(res) || typeof res === 'boolean')
                                return setFatalError('bad value in '+op+': '+res, cb2, undefined);
                            if (Decimal.isDecimal(res))
                                res = toDoubleRange(res);
                            evaluated_params[param_name] = {
                                operator: params[param_name].operator,
                    ...
                    function(param_name, cb2){
                        evaluate(params[param_name].value, function (res) {
                            ...
                            if (typeof res !== 'string' && param_name !== 'ifnone')
                                return setFatalError('bad value of '+param_name+' in attestation: '+res, cb2, undefined);
                            if (Decimal.isDecimal(res)) {
                                if (!isFiniteDecimal(res))
                                    return setFatalError('not finite '+param_name+' in attestation: '+res, cb2, undefined);
                                res = toDoubleRange(res);
                            }
                            evaluated_params[param_name] = {
                                operator: params[param_name].operator,
                ...
                async.eachSeries(arr.slice(1), function (param, cb2) {
                    evaluate(param, function (res) {
                        ...
                        if (res instanceof wrappedObject)
                            ...
                            return setFatalError('unrecognized type in ' + op + ': ' + res, cb2, undefined);
                    ...
                    function (item_expr, cb2) {
                        evaluate(item_expr, function (res) {
                            ...
                            if (res instanceof wrappedObject) {
                                ...
                                if (!isValidValue(res))
                                    return setFatalError("bad value " + res, cb2, undefined);
                                if (Decimal.isDecimal(res))
                                    res = res.toNumber();
                                arrItems.push(res);
                            }
    ...
    function setFatalError(err, cb, cb_arg = undefined){
        fatal_error = err;
        console.log(err);
