...
const origNext = lexer.next;

lexer.next = function () {
    const tok = origNext.call(this);
    if (tok) {
        switch (tok.type) {
            case 'WS':
            case 'comment':
...
const grammar = {
    ...
    ParserRules: [
        ...
        {"name": "AS", "symbols": ["AS", {"literal":"-"}, "MD"], "postprocess": function(d) {return ['-', d[0], d[2]]; }},
        {"name": "AS", "symbols": [{"literal":"-"}, "MD"], "postprocess": function(d) {return ['-', 0, d[1]]; }},
        {"name": "AS", "symbols": [{"literal":"+"}, "MD"], "postprocess": function(d) {return ['+', 0, d[1]]; }},
        ...
        {"name": "float", "symbols": [(lexer.has("number") ? {type: "number"} : number)], "postprocess": function(d) { return d[0].value; }},
